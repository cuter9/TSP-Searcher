...
def tabu_search(distance_matrix, city_tour, iterations=150, tabu_tenure=20, verbose=True):
    ...
    no_improvement = 0
    count_lst = [count]
    cost_lst = [np.min(best_solution[1])]

    while count < iterations:
        if verbose == True:
            print('Iteration = ', count, 'Distance = ', round(best_solution[1], 2))
        stm_and_ltm, city_tour, tabu_list = tabu_update(distance_matrix, stm_and_ltm, city_tour, best_solution[1],
                                                        tabu_list, tabu_tenure, diversify)
        ...
        cost_lst.append(round(np.min(best_solution[1]), 2))
