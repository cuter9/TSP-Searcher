...
def simulated_annealing_tsp(distance_matrix, initial_temperature=1.0, temperature_iterations=10,
                            final_temperature=0.0001, alpha=0.9, verbose=True):
    guess = seed_function(distance_matrix)
    best = copy.deepcopy(guess)
    # best = np.copy(guess)
    temperature = float(initial_temperature) * 0.9 * guess[1]
    final_temperature = float(final_temperature) * temperature
    fx_best = float('+inf')
    count = 0
    count_lst = [count]
    cost_lst = [np.min(best[1])]
    temperature_lst = [temperature]
    while temperature > final_temperature:
        for repeat in range(0, temperature_iterations):
            if verbose == True:
                print('Temperature = ', round(temperature, 4), ' ; iteration = ', repeat, ' ; Distance = ',
                      round(best[1], 2))
            fx_old = guess[1]
            ...
            if fx_new < fx_best:
                fx_best = fx_new
                best = copy.deepcopy(new_guess)
                # best = np.copy(guess)
        temperature = alpha * temperature
        count += 1

        count_lst.append(count)
        cost_lst.append(round(np.min(best[1]), 2))
        temperature_lst.append(temperature)
    # route, distance = guess
    best_solution = guess
    # best_solution = [route, distance]
    evolution_profile = [count_lst, cost_lst, temperature_lst]
