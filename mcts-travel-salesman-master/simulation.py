...
def simulation(num_of_network, trail_per_network, num_of_node, side_length=100, plot=False):
    ...
    if plot is True:
        ...
        fig, ax = plt.subplots(nrows=1, ncols=1, figsize=(8, 8))
        ax.hist(h_1[:, 0], bins=range(bin_start, bin_end, 20), density=True, alpha=0.6, color='r',
                label='greedy heuristic')
        ax.hist(h_2[:, 0], bins=range(bin_start, bin_end, 20), density=True, alpha=0.6, color='m',
                label='two opt heuristic')
