...
def two_opt(network):
    ...
    while counter < len(path_edges) * 10:  # when path is stable, terminate
        ...
        # swap the nodes between the selected_edges, from start node to start node, and end node to end node
        ...
        if new_cost < current_cost:  # perform the path switch using the new edges, once if the swap result a better cost
            # switch the two selected edges
            path_edges[selected_edge_indexs[0]] = new_edges[0]
            path_edges[selected_edge_indexs[1]] = new_edges[1]

            # reorder the edges in between the nodes of the new edges
            if abs(selected_edge_indexs[0] - selected_edge_indexs[1]) > 1:
                path_edges[selected_edge_indexs[0] + 1:selected_edge_indexs[1]] = path_edges[
                                                                                  selected_edge_indexs[1] - 1:
                                                                                  selected_edge_indexs[0]:-1]
