...
def rw_selection(pop, costs, max_tnm):  # roulette-wheel selection, max_tnm : only needed in tournament selection
    costs = [max(costs) - x for x in costs]  # avoid negative scores
    sum_score = sum(costs)
    return pop[np.random.choice(len(pop), p=[x / sum_score for x in costs])]  # len(pop) = np.arange(len(pop))
...
def elt_selection(pop, costs, max_tnm):  # elitism, , max_tnm : only needed in tournament selection
...
def crossover(p_1, p_2, r_cross):
    ...
    if random.random() < r_cross:
        ...
        for _ in range(pt_2 - pt_1):
            c1.remove(p_2[pt_1 + _])    # remove the elements of p_2 from p_1, then insert with the seg of p2
            c2.remove(p_1[pt_1 + _])    # remove the elements of p_1 from p_2, then insert with the seg of p1
...
def ga(n, adj_mat, n_pop, r_cross, r_mut, selection_md, max_tnm, term_count):
    ...
    data = {'cost': deque([]), 'best_cost': deque([]),
            'sol': deque([]), 'best_sol': deque([])}  # cost means avg_cost
    count = 0
    # select parents
    if selection_md == 'tnm':
